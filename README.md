...
- `.bar` — глобальный ярус (48px): бренд слева, экраны и аккаунт справа.
  От проекта не зависит.
- `.project-bar` — проектный ярус (44px): открытый проект, сводка чипами и
  техническое меню выгрузки. Лежит внутри `#workspace`, поэтому без проекта
  его нет вовсе, а не спрятан.
- `.rail` — разделы конфигурации слева. В разметке они остаются в контракте
  `.tabs button[data-view]`, на который опираются `app.js` и браузерный тест.
- `.pane` — центральная панель: один ряд `.toolbar` и прокручиваемое
  `.pane-body`. Прокручиваются панели, страница — нет.
...
- `.veil` / `.sheet` — листы для разовых действий: настройка отчёта и
  пропуски по анкете. Это не разделы панели, поэтому заполнять её ширину
  они не обязаны.

Третья колонка появляется через `.shell:has(.insp:not([hidden]))` — состояние
живёт в CSS, чтобы `app.js` не синхронизировал класс из десятка мест, где
редакторы прячутся и показываются.
