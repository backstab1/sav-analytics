...
- `.bar` — единственный ярус хрома (42px): бренд, открытый проект со сводкой
  чипами, экраны, меню выгрузки и аккаунт. Проектное лежит в нём отдельными
  блоками `#project-chrome` и `#project-actions`; без открытого проекта они
  скрыты целиком, показывает и прячет их `Shell.setProjectOpen`.
...
- `.pane` — окно со списком: не «вся ширина минус рельс», а карточка шириной
  `min(100% - 2 × --canvas-pad, --canvas-max)` по центру холста, вокруг неё
  поле. Внутри один ряд `.toolbar` и прокручиваемое `.pane-body`.
  Прокручиваются панели, страница — нет.
- `.insp` — единый инспектор для всех пяти сущностей: голова, тело со своей
  прокруткой, ряд действий. Высота считается по содержимому
  (`align-self: start` + `max-height: 100%`), а не задаётся константой.
- `.veil` / `.sheet` — листы для разовых действий: настройка отчёта и
  пропуски по анкете. Выезжают с той же стороны, что и инспектор, встык
  к рельсу и под шапкой.

Всё, что настраивают, приходит слева, а справа остаётся поле: инспектор
и листы стоят между рельсом и окном списка, а не поверх него и не третьей
