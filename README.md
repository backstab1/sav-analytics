...
- `.bar` — единственный ярус хрома (48px): бренд, экраны, открытый проект,
  сводка чипами, выгрузка, настройки. Всё, что относится к проекту, помечено
  `.project-only` и показывается, пока открыт `#workspace`.
